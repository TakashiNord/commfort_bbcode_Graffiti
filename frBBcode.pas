...
procedure TBBcodeForm.FormDestroy(Sender: TObject);
begin
  //данная функция вызывается при завершении работы плагина
  //if Assigned(GraffitiForm) then begin
    GraffitiForm.Close ;
    GraffitiForm.Free ;
  //FreeAndNil(GraffitiForm);
  //end;
end;
...
procedure TBBcodeForm.Button6Click(Sender: TObject);
begin
  try
   //данная функция вызывается при завершении работы плагина
  if Not Assigned(GraffitiForm) then begin
    GraffitiForm := TGraffitiForm.Create(nil);
    GraffitiForm.Visible:=False ;
  end;
     GraffitiForm.Show() ;
  finally
   ;
...
procedure TBBcodeForm.Button7Click(Sender: TObject);
begin
 //
end;
